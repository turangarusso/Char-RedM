...
RegisterServerEvent('ruxo:AzioneChat')
AddEventHandler('ruxo:AzioneChat', function(source, args, raw)
	AzioneCommand(source, args, raw)
end)

RegisterServerEvent('ruxo:ChatAnnuncio')
AddEventHandler('ruxo:ChatAnnuncio', function(source, args, raw)
	AnnuncioCommand(source, args, raw)
end)
...
--[[ RegisterCommand('azione', function(source, args, raw)
	AzioneCommand(source, args, raw)
end) ]]

function AzioneCommand(source, args, raw)
	local message = table.concat(args, ' ')
	AzioneMessage(source, message)
end

function AnnuncioCommand(source, args, raw)
	local message = table.concat(args, ' ')
	AnnuncioMessage(source, message)
end

function AnnuncioMessage(source, message)
	if message == '' then
		return
	end
    ...
    if Character.job ~= "unemployed" then
        local name = GetPlayerName(source)
        TriggerClientEvent('ruxo:chatAnnuncio', -1, source, message, name)
    end
end


function AzioneMessage(source, message)
	if message == '' then
		return
	end
    local name = GetPlayerName(source)
	TriggerClientEvent('ruxo:chatAzione', -1, source, message, name)
